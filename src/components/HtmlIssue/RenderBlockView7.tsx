...
import RenderBlockView8 from './RenderBlockView5'
...
function RenderBlockView7 (props: NodeType & {onClick: (src) => void}) {
  ...
  return (
    <View className={tag} style={attrs.style}>
      ...
      {
        nodes.filter(v => InlineTags.indexOf(v.tag) < 0).map((node, index) => <RenderBlockView8 key={index} onClick={onClick} tag={node.tag} attrs={attrs} nodes={node.nodes} />)
      }
    </View>
  )
}
