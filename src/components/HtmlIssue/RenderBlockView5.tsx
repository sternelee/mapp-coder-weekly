...
import RenderBlockView6 from './RenderBlockView6'
    ...
    <View className={tag} style={attrs.style}>
      {
        ...
        nodes.map((node, index) => {
          ...
          else if (node.tag !== 'text') return <RenderBlockView6 key={index} onClick={onClick} tag={tag} attrs={attrs} nodes={nodes} />
          else return null
          // else return <RenderBlockView6 key={index} onClick={onClick} tag={tag} attrs={attrs} nodes={nodes} />
        })
      }
    </View>
  )
