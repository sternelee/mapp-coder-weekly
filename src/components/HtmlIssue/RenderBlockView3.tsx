...
function RenderBlockView3 (props: NodeType & {onClick: (src) => void}) {
  ...
  return (
    <View className={tag} style={attrs.style}>
      ...
      {
        nodes.filter(v => InlineTags.indexOf(v.tag) < 0).map((node, index) => <RenderBlockView4 key={index} onClick={onClick} tag={node.tag} attrs={attrs} nodes={node.nodes} />)
      }
    </View>
  )
}
